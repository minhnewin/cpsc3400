    ...
    def isDescendant(self,personID):
        ...
        if self._id != personID:
            for fam in self._asSpouse:
                b = families[fam].isInFamily(personID)
                if b == True:
                    break
        else:
...
class Family():
    ...
    def isInFamily(self,personID):
        for child in self._children:
            b = persons[child].isDescendant(personID)
            if b == True:
                return True
            else:
                return False
...
##    filename = input("Type the name of the GEDCOM file:")
    ...
    person1 = "I46"  # Default selection to work with Kennedy.ged file
    person2 = "I63" # tester for descendant
##    Uncomment the next line to make the program interactive
##    person = input("Enter person ID for descendants chart:")

    getPerson(person1).printDescendants()

    if getPerson(person1).isDescendant(person2):
        print("is descendant\n")
    else:
